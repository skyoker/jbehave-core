...
public class StepCandidateBehaviour {
    ...
    public void shouldCreatePerformableStepWithResultThatDescribesTheStepPerformed() throws Exception {
        ...
        StepResult result = candidate.createMatchedStep(stepAsString, namedParameters)
    ...
    private void performStep(StepCandidate candidate, String stepAsString) {
        StoryReporter reporter = mock(StoryReporter.class);
        candidate.createMatchedStep(stepAsString, namedParameters).perform(reporter, null);
        verify(reporter).beforeStep(stepAsString);
    }
    ...
    public void shouldCaptureOutcomeFailures() {
        ...
        StepResult stepResult = candidates.get(0).createMatchedStep(stepAsString,
                namedParameters).perform(reporter, null);
    ...
    public void shouldRestart() {
        ...
        StoryReporter reporter = mock(StoryReporter.class);
        try {
            candidates.get(0).createMatchedStep(stepAsString, namedParameters)
                    .perform(reporter, null);
    ...
    public void shouldPerformStepsInDryRunMode() {
        ...
        step0.createMatchedStep(stepAsString00, namedParameters).perform(reporter, null);
        String stepAsString01 = "And foo named xyz";
        step0.createMatchedStep(stepAsString01, namedParameters).perform(reporter, null);
        StepCandidate step1 = candidateMatchingStep(candidates, "When foo named $name");
        String stepAsString10 = "When foo named Bar";
        step1.createMatchedStep(stepAsString10, namedParameters).perform(reporter, null);
        String stepAsString11 = "And foo named Bar";
        step1.createMatchedStep(stepAsString11, namedParameters).perform(reporter, null);
    ...
    public void shouldNotCreateStepOfWrongType() {
        ...
        step.createMatchedStep(stepAsString0, namedParameters).perform(reporter, null);
        assertThat(steps.givenName, equalTo("xyz"));
        assertThat(steps.whenName, nullValue());
        verify(reporter).beforeStep(stepAsString0);
        String stepAsString1 = "Then foo named xyz";
        step.createMatchedStep(stepAsString1, namedParameters).perform(reporter, null);
